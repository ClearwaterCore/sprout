...
/// Utility method to create a UASTsx object for unsolicited requests.
void SproutletProxy::create_uas_tsx(
    pjsip_tx_data* tdata,
    Sproutlet* sproutlet,
    ...
    void* user_data)
{
  SproutletProxy::IntUASTsx* uas_tsx =
    new SproutletProxy::IntUASTsx(this, terminated_callback, user_data);
  uas_tsx->init(tdata, sproutlet, alias, trail);
}
...
void SproutletProxy::UASTsx::forward_tx_response(SproutletWrapper* downstream,
                                                 pjsip_tx_data* rsp)
{
                                            ...
                                            pjsip_tx_data* rsp)
{
  ...
  {
    forward_tx_response(downstream, rsp);
  }
...
SproutletProxy::IntUASTsx::IntUASTsx(
  ...
  SproutletProxy::UASTsx(proxy),
  _terminated(false),
  _terminated_callback(terminated_callback),
...
///
/// On failure this method deletes the object.
void SproutletProxy::IntUASTsx::init(pjsip_tx_data* req,
                                     Sproutlet* sproutlet,
                                     const std::string& alias,
                                     SAS::TrailId trail)
{
  ...
  if (status != PJ_SUCCESS)
  {
    ...
    if (_terminated_callback != NULL)
    {
      _terminated_callback(_user_data);
    }
    delete this;
    // LCOV_EXCL_STOP
  }
  ...
  // Schedule any requests generated by the Sproutlet.  This may result in our
  // own destruction.
  schedule_requests();
  check_destroy();
  exit_context();
                                            ...
                                            pjsip_tx_data* rsp)
{
  if (downstream == _root)
  {
    // This is the root sproutlet in the tree.  If this is a final response
    // then it terminates the transaction; either way, we drop it.
    if (rsp->msg->line.status.code >= PJSIP_SC_OK)
    {
      if (_terminated_callback != NULL)
      {
...
/// the IntUASTsx when all the Sproutlets have completed their processing, which
/// only occurs when all the linkages are broken.
void SproutletProxy::IntUASTsx::check_destroy()
{
  ...
  {
    // UAS transaction has been destroyed and all Sproutlets are complete.
    TRC_DEBUG("Safe for IntUASTsx to suicide");
    on_tsx_complete();
    _pending_destroy = true;
  }
}
