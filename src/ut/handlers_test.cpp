...
TEST_F(AoRTimeoutTasksTest, RemoteAoRNoBindingsTest)
{
...

  {
    InSequence s;
      ...
      EXPECT_CALL(*remote_store, set_aor_data(aor_id, _, remote_aor_pair, _, _, _, _))
...
TEST_F(AoRTimeoutTasksTest, LocalAoRNoBindingsTest)
{
...

  {
    InSequence s;
      ...
      EXPECT_CALL(*remote_store, set_aor_data(aor_id, _, remote_aor_2, _, _, _, _)).WillOnce(Return(Store::OK));
  }
...
TEST_F(AoRTimeoutTasksTest, NoBindingsTest)
{
...

  {
    InSequence s;
      ...
      EXPECT_CALL(*remote_store, has_servers()).WillOnce(Return(true));
      EXPECT_CALL(*remote_store, get_aor_data(aor_id, _)).WillOnce(Return(remote_aor_pair));
      EXPECT_CALL(*store, set_aor_data(aor_id, _, aor_pair, _, _, _, _)).WillOnce(DoAll(SetArgReferee<4>(true), Return(Store::OK)));
      EXPECT_CALL(*remote_store, has_servers()).WillOnce(Return(true));
      EXPECT_CALL(*mock_hss, get_registration_data(_, _, _, _, _))
           .WillOnce(DoAll(SetArgReferee<3>(std::vector<std::string>(irs_impus)), //IMPUs in IRS
                           Return(HTTP_OK)));
      EXPECT_CALL(*remote_store, get_aor_data(aor_id, _)).WillOnce(Return(remote_aor_pair2));
      EXPECT_CALL(*remote_store, set_aor_data(aor_id, _, remote_aor_pair2, _, _, _, _)).WillOnce(DoAll(SetArgReferee<4>(true), Return(Store::OK)));
